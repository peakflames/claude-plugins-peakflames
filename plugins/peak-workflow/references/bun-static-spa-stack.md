...
| Secrets | **None** | Anything in the bundle is public. A product needing a secret needs a server. |
...
```json
{
  "compilerOptions": {
    ...
    "skipLibCheck": true,
    "jsx": "react-jsx",
    "types": ["bun-types", "vite/client", "vite-plugin-pwa/client"],
    "baseUrl": ".",
    "paths": { "@/*": ["./src/*"] }
  },
...
```json
{
  ...
  "files": { "ignore": ["dist", "dev-dist", "src/components/ui", "src/routeTree.gen.ts"] },
...
// Example domain — substitute your own.
...
import type { Habit, Completion } from "./schema";

export const db = new Dexie("my-app") as Dexie & {
...
db.version(1).stores({
  ...
  completions: "id, habitId, day, [habitId+day]",
});
```

The compound index `[habitId+day]` is what makes "is this done today?" and streak scans cheap.
...
import { Habit, Completion } from "./schema";
...
export async function importBackup(file: File): Promise<void> {
  ...
  await db.transaction("rw", db.habits, db.completions, async () => {
    ...
    await db.completions.bulkAdd(parsed.completions);
  });
}
```
...
console.info(`my-app v${__APP_VERSION__} starting`);
...
export function HabitList({ showArchived }: { showArchived: boolean }) {
  const habits = useLiveQuery(
    () => (showArchived ? db.habits.toArray() : db.habits.filter((h) => !h.archivedAt).toArray()),
    [showArchived],
  );

  if (habits === undefined) return <Spinner />;   // loading
  if (habits.length === 0) return <EmptyState />; // empty
...
Storing a derived value is how a streak count goes wrong: it drifts from the completions that
...
test("counts consecutive days ending today", async () => {
  await db.completions.bulkAdd([
    { id: crypto.randomUUID(), habitId: "h1", day: "2026-03-08" },
    { id: crypto.randomUUID(), habitId: "h1", day: "2026-03-09" },
  ]);
  const marks = await db.completions.where({ habitId: "h1" }).toArray();
...
test.beforeEach(async ({ page }) => {
  await page.goto("/");
  await page.evaluate(() => indexedDB.deleteDatabase("my-app"));
  await page.reload();
});
```
...
|---|---|---|
| Deep link 404s on a static host | `/habits` has no file behind it; Pages returns its 404 page. | Hash history (6.1) — the path after `#` never reaches the server. |
...
| Derived values drifting | A stored streak count disagrees with the completions. | Compute in `services/`, never persist (6.3). |
