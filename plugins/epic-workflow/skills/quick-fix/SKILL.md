...
description: |
  ...
  Use when `/epic-workflow:triage` returned TRIVIAL, or when you know the request is too small
...
You are implementing a small fix via the lightweight quick-fix path. Unlike `/epic-workflow:start`, this path does **not** create an epic spec, does **not** touch `docs/implementation-plan/index.md`, and does **not** write a handoff file. The commit message (and optional PR) is the permanent record.
...
6. **Ship** — branch on the ship mode chosen in Step 2:
   ...
   - Body template (populate from the issue body, quality gate results, and the manual-verification value captured in Step 4):
     ...
     **Manual verification performed:** <Yes | No>
     ...
     🤖 Generated via /epic-workflow:quick-fix
     ```
